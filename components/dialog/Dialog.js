    ...
    return (
        <div style={{ display: 'flex' }}>
            <MUIDialogTitle
                disableTypography
                {...other}
                style={{ display: 'flex', flexDirection: 'row', width: '100%', padding: '16px 24px 0px 24px' }}
            >
    ...
    render() {
        ...
        return (
            <DialogContainer open={this.props.open || false}>
                ...
                <div style={{ flexGrow: 1, padding: '0px 30px 0px 30px', overflow: 'auto' }}>{this.props.children}</div>
            </DialogContainer>
        );
    }
}
